...
MODEL_CONFIG = {
    ...
    "HEIGHT_ROOF_L": {"min": 0, "max": 30},
    "HEIGHT_ROOF_R": {"min": 0, "max": 30},
    "WIDTH_ROOF_L": {"min": 5, "max": 50},
    "WIDTH_ROOF_R": {"min": 5, "max": 50},
    "LENGTH": {"min": 20, "max": 100}
}

SAME_HEIGHT_PORTION = 0.4
SAME_WIDTH_PORTION = 0.3
...
EXPORT_ROOT = os.path.join(DATASET_ROOT, "Simple-House-2\\{model_id}")
...
class HouseModelGenerator():
    ...
    def _reset(self, delete_prev=True):
        ...
        self.point_up_mid_left  = rs.AddPoint(0, 0, 0)
        self.point_up_mid_right = rs.AddPoint(0, 0, 0)
    ...
    def generate(self):
        ...
        if model_param["width_roof_r"] < model_param["width_r"]:
            

        model_param["width_roof_r"] = min(model_param["width_r"], model_param["width_roof_r"])
        model_param["width_roof_l"] = max(model_param["width_l"], model_param["width_roof_l"])

        self.model_param = model_param
        
        # bottom - left
        bl = (-model_param["width_l"], 0, 0)
        rs.PointCoordinates(self.point_bottom_left, bl)
        
        # bottom - right
        br = (model_param["width_r"], 0, 0)
        rs.PointCoordinates(self.point_bottom_right, br)
        
        # up - left
        ul = (-model_param["width_l"], 0, model_param["height_l"])
        rs.PointCoordinates(self.point_up_left, ul)
        
        # up - mid - left
        uml = (model_param["width_l"], 0, self.roof_height)
        rs.PointCoordinates(self.point_up_mid_left, uml)

        self.roof_height_l = (model_param["height_l"] + model_param["height_r"]) / 2 + model_param["height_roof"]
        ...
        # remove guided shapes
        rs.DeleteObjects([self.bounding_edge, self.extrude_path])
        
        # determine center point
        cx = (model_param["width_l"] + model_param["width_r"]) / 2
        cy = (model_param["length"]) / 2
        cz = (max(ul[2], um[2], ur[2])) / 2
        
        self.center_point = Rhino.Geometry.Point3d(cx, cy, cz)

        # move the model to center 
        xform = rs.XformTranslation(-self.center_point)
        rs.TransformObjects( self.house_model, xform, False )
        
        self.center_point = Rhino.Geometry.Point3d(0, 0, 0)

        sc.doc = ghdoc

        return model_param

    # Deprecated: replaced by `self.center_point`
...
for model_id in range(MODEL_COUNT):
    ...
    with open(EXPORT_VIEW_TXT_FILENAME.format(model_id=model_id), 'w') as view_txt:
        for sketch_id in range(SKETCHES_PER_MODEL):
            azimuth = random.randint(20, 70) + 90 * random.randint(0, 3)
            if azimuth > 180:
